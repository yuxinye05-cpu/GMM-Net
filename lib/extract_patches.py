...
def recompone_overlap(preds, img_h, img_w, stride_h, stride_w):
    """
    Recompose overlapping patches into full images.
    preds: (N_patches, C, patch_h, patch_w)
    Returns: (N_full_imgs, C, img_h, img_w) with pixel values clipped to [0,1]
    """
    assert len(preds.shape) == 4  # 4D array
    ...
    print(f"There are {N_full_imgs} images in Testset")
    ...
    # 调试信息
    print(f"full_sum.min()={full_sum.min()}, full_sum.max()={full_sum.max()}")

    # 安全除法，避免 NaN/Inf
    with np.errstate(divide='ignore', invalid='ignore'):
        final_avg = np.true_divide(full_prob, full_sum)
        final_avg[~np.isfinite(final_avg)] = 0

    # Clip 到 [0,1]
    final_avg = np.clip(final_avg, 0.0, 1.0)

    print(f"After recompone_overlap: min={final_avg.min()}, max={final_avg.max()}, "
          f"NaN={np.isnan(final_avg).sum()}, Inf={np.isinf(final_avg).sum()}")

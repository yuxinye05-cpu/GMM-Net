...
from torchvision import transforms    # 图像预处理方法
    ...
    def __init__(self, patches_imgs,patches_masks,mode="train"):
        ...
        if mode == "train":
            ...
            self.transforms = Compose([
                ...
                #RandomCrop((64, 64)),        # 随机裁剪
                RandomFlip_LR(prob=0.5),     # 随机水平翻转
                RandomFlip_UD(prob=0.5),     # 随机垂直翻转
                RandomRotate()               # 随机旋转90度的倍数
            ])
...
class RandomCrop:
    def __init__(self, shape):
        self.shape = [shape, shape] if isinstance(shape, int) else shape     # 输入图片的尺寸
        self.fill = 0
    ...
    def _get_range(self, shape, crop_shape):
        if shape == crop_shape:
            ...
            start = random.randint(0, shape - crop_shape)    # 随机返回一个整数，确定裁剪位置的起点
        end = start + crop_shape                             # 裁剪位置的终点
        return start, end

    def __call__(self, img, mask):           # 使得类实例对象可以像调用普通函数那样，以"对象名()"的形式使用。
        _, h, w = img.shape
        sh, eh = self._get_range(h, self.shape[0])     # 确定裁剪位置
        sw, ew = self._get_range(w, self.shape[1])
        return img[:, sh:eh, sw:ew], mask[:, sh:eh, sw:ew]     # 进行裁剪
...
class RandomFlip_LR:
    ...
    def __call__(self, img, mask):
        prob = (random.uniform(0, 1), random.uniform(0, 1))        # 随机生成实数，在[0,1]之间

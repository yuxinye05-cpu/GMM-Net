...
class EfficientAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, group_split: List[int], kernel_sizes: List[int], window_size=7,
                 attn_drop=0.1, proj_drop=0.1, qkv_bias=True):
        super().__init__()
        ...
        for i in range(len(kernel_sizes)):
            ...
            convs.append(nn.Conv2d(3 * self.dim_head * group_head, 3 * self.dim_head * group_head, kernel_size,
                                   1, kernel_size // 2, groups=3 * self.dim_head * group_head))   #深度可分离卷积
...
class EfficientBlock(nn.Module):
    ...
    def __init__(self, dim, out_dim, num_heads, group_split: List[int], kernel_sizes: List[int], window_size: int,
                 mlp_kernel_size: int, mlp_ratio: int, stride=1, attn_drop=0.1, mlp_drop=0.25,lfau_drop=0.25,qkv_bias=True,
                 drop_path=0.1):
        super().__init__()
        self.dim = dim
        self.mlp_ratio = mlp_ratio
        self.norm1 = nn.GroupNorm(1, dim)  # 层标准化
    ...
    def forward(self, x: torch.Tensor):
        ...
        # x = self.downsample(x) + self.mlp(self.norm2(x))
        return x


if __name__ == '__main__':
    input = torch.randn(4, 256, 16, 16)
    model = EfficientBlock(256, 256, 8, [4,4], [3], 2, 3, 4)
